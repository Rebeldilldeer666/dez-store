...
;(async()=>{
  ...
  for(const p of list){
    if(p.title.toLowerCase().includes("polyester")||p.title.toLowerCase().includes("aop")){
      console.log("Deleting",p.id,p.title)
      await api(`/shops/${shopId}/products/${p.id}.json`,{method:"DELETE"})
    }
  }
  console.log("Deleted AOP junk")
})()
