...
- **Confirmed mismatch:** the project runtime is authenticated as `acct_1UCi6xBEWwFFjQw7` in **test mode** and currently returns 0 active products and 0 active prices. The screenshot showing 422 active products is from a different Stripe account or mode. This cannot be corrected by code alone without changing the connected credential.

## Verification procedure

1. Open `/api/products` in production.
2. Compare `stripeAccount`, `livemode`, and `total` with the Stripe dashboard account and mode. The current runtime values are `acct_1UCi6xBEWwFFJQw7`, `false`, and `0`.
