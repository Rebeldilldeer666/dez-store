...
type CatalogMeta = { total: number; purchasable: number; missingDefaultPrice: number; stripeAccount?: string; livemode?: boolean }
  ...
  useEffect(() => {
    ...
    fetch('/api/products', { signal: controller.signal })
      .then(async (response) => {
        ...
        setCatalog(data)
        setStatus(data.products?.length ? '' : data.total ? `Catalog connected: ${data.total} active products, ${data.missingDefaultPrice} missing a default price.` : 'The catalog is connected but has no active products.')
      })
